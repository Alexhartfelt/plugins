...
function activate() {
	...
	function handle_filter_button() {
		const tabWrapper = document.querySelector("#hr-products .hr-tab-wrapper");
		if (!tabWrapper) {
			return;
		}
		const totalProducts = tabWrapper.dataset.totalProducts;
		...
		filterButtons.forEach(button => {
			if (totalProducts <= 0 || searcher.search_term.length <= 0) {
				...
				button.closest(".hr-header") ? button.disabled = false : button.style.display = "";
			}
		});
	}
