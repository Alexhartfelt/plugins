...
Show a **diff of just the regions you changed**, labeled by field — not the whole multi-KB field. Offer full files on request. **Wait for an explicit "go ahead."** Never write customer output to disk; never overwrite `${CLAUDE_PLUGIN_ROOT}/docs/wiki/base-templates/`.
...
Then add a short build retro: how many diff/approval rounds it took, what the operator corrected (categories, not full quotes — e.g. "swiper breakpoint," "ATC hook re-init"), and whether anything landed in `${CLAUDE_PLUGIN_ROOT}/docs/wiki/cheat-sheets/` as a result. Keep it to 2-4 bullets, chat-only.
...
3. **Show your changes and get explicit approval before touching the live config; never write to disk.** Diff vs the design read with `recoms_getDesign`; on approval push a **REVIEW draft** via `recoms_updateDesign` (never publish; never auto-push). With no MCP coordinates, fall back to inline copy-paste.
